...
import game.wrapper as bridgegame
import bridge.game as origgame
import bridge.lin as lin
# import scaffold.fsa as fsa
...
import game as rustgame
class RustDeal:
    def __init__(self):
        self.dealer = None
        self.vulnerability = rustgame.Vulnerability("None")
        self.rsdeal = None
        self.error = None
        self.result = None
    ...
    def setup(self) -> Self:
        if not self.rsdeal:
            self.rsdeal = rustgame.Deal(self.dealer, self.vulnerability)
            
    def set_dealer(self, dealer: str) -> Self:
        self.dealer = rustgame.Seat(dealer)
        return self

    def set_vulnerability(self, vulnerability: str) -> Self:
        self.vulnerability = rustgame.Vulnerability(vulnerability)
        return self
    
    def next_to_act(self) -> str:
        nxt = self.rsdeal.next_to_act()
        if nxt:
            return nxt.as_string()
        return None
    
    def deepcopy(self) -> Self:
        d = copy.copy(self)
        d.rsdeal = d.rsdeal.deepcopy()
        return d
    
    def pass_position(self) -> int:
        return self.rsdeal.pass_position()
    
    def num_actions(self) -> int:
        return self.rsdeal.num_actions()
    
    def contract_strain(self) -> str:
        return self.rsdeal.contract_strain()
    
    def contract_level(self) -> str:
        return self.rsdeal.contract_level()
    
    def contract_seat(self) -> str:
        return self.rsdeal.contract_seat()
    
    def trick_counts(self) -> dict:
        won = self.rsdeal.tricks_won()
        return {
            "South": won[0],
            "West": won[1],
            "North": won[2],
    ...
    def action(self, id: int) -> int:
        return self.rsdeal.action(id)

class RustGame:
    def Deal(self) -> RustDeal:
        return RustDeal()

    def set_players(self, deal: RustDeal, *args, **kwargs) -> RustDeal:
        return deal
    
    def set_board_number(self, deal: RustDeal, n: int) -> RustDeal:
        ...
        deal.vulnerability = rustgame.Vulnerability(vuln)
        return deal
    
    def copy_replay_state(self, deal, *args, **kwargs):
        return deal  #FIXME
    
    def set_result(self, deal, *args, **kwargs):
        deal.setup()
        return deal #FIXME

    def give_card(self, deal: RustDeal, seat, suit, rank) -> RustDeal:
        deal.setup()
        try:
            deal.rsdeal.give_card(rustgame.Seat(seat), rustgame.Card(suit, rank))
        except TypeError as e:
            deal.error = e    
        return deal
            
    def make_bid(self, deal: RustDeal, level, strain) -> RustDeal:
        deal.setup()
        bid = rustgame.Bid(rustgame.Level(level), strain)

        try:
            deal.rsdeal.execute_bid_action(bid)
        except TypeError as e:
            deal.error = e
        return deal
    
    def kibitzer_view(self, deal, *args, **kwargs):
        return deal  #FIXME


    def set_dealer(self, deal: RustDeal, dealer: str) -> RustDeal:
        deal = deal.set_dealer(dealer)
        return deal

    def make_call(self, deal: RustDeal, call: str):
        deal.setup()
        try:
            deal.rsdeal.execute_call_action(rustgame.Call(call))
        except TypeError as e:
            deal.error = e

        return deal
    
    def play_card(self, deal: RustDeal, suit: str, rank: str) -> RustDeal:
        deal.setup()
        try:
            deal.rsdeal.execute_play_action(rustgame.Card(suit, rank))
        except TypeError as e:
            deal.error = e

        return deal
    
    def execute_action(self, deal: RustDeal, action) -> RustDeal:
        deal.setup()
        try:
            deal.rsdeal.execute_actions_ids(bytes([action]))
        except TypeError as e:
            deal.error = e
...
class GameTest(absltest.TestCase):
    ...
    def assertDealEqual(self, actual, expected):
        ...
        print(actual.rsdeal.show())
        print(expected.rsdeal.show())
        self.assertTrue(actual.rsdeal.equals(expected.rsdeal))
    ...
    def test_sufficient(self):
        ...
        fail = self.game.make_bid(deal.deepcopy(), "2", "Clubs")
        self.assertNotEqual(fail.error, None)
        fail = self.game.make_bid(deal.deepcopy(), "2", "Diamonds")
        self.assertNotEqual(fail.error, None)
        fail = self.game.make_bid(deal.deepcopy(), "2", "Hearts")
        self.assertNotEqual(fail.error, None)
        fail = self.game.make_bid(deal.deepcopy(), "1", "Hearts")
        self.assertNotEqual(fail.error, None)
        ok = self.game.make_bid(deal.deepcopy(), "2", "Spades")
        self.assertEqual(ok.error, None)
        ok = self.game.make_bid(deal.deepcopy(), "3", "Hearts")
        self.assertEqual(ok.error, None)
        ok = self.game.make_bid(deal.deepcopy(), "3", "Clubs")
        self.assertEqual(ok.error, None)
    ...
    def test_double(self):
        ...
        dealP = self.game.make_call(deal.deepcopy(), "pass")
        dealPP = self.game.make_call(dealP.deepcopy(), "pass")
        dealPPX = self.game.make_call(dealPP.deepcopy(), "double")
        dealPPXP = self.game.make_call(dealPPX.deepcopy(), "pass")
        dealPPXPP = self.game.make_call(dealPPXP.deepcopy(), "pass")
        dealPPXPPR = self.game.make_call(dealPPXPP.deepcopy(), "redouble")
        dealPPXPPRP = self.game.make_call(dealPPXPPR.deepcopy(), "pass")

        dealR = self.game.make_call(deal.deepcopy(), "redouble")
        dealPX = self.game.make_call(dealP.deepcopy(), "double")
        dealPR = self.game.make_call(dealP.deepcopy(), "redouble")
        dealPPXPX = self.game.make_call(dealPPXP.deepcopy(), "double")
        dealPPXPR = self.game.make_call(dealPPXP.deepcopy(), "redouble")
        dealPPXPPRPX = self.game.make_call(dealPPXPPRP.deepcopy(), "double")
        dealPPXPPRPR = self.game.make_call(dealPPXPPRP.deepcopy(), "redouble")
    ...
    def test_execute_action(self):
        deal = self.game.Deal()
        deal = self.game.set_dealer(deal, "South")
        deal = self.game.set_result(deal, "1", "Hearts", "Sosetuth", "undoubled", "=")
        dealH = self.game.make_bid(deal.deepcopy(), "1", "Hearts")
        print(dealH.action(0))
        dealA = self.game.execute_action(deal.deepcopy(), dealH.action(0))
        print(dealA.action(0))
        self.assertDealEqual(dealA, dealH)
        dealHP = self.game.make_call(dealH.deepcopy(), "pass")
        dealHA = self.game.execute_action(dealH.deepcopy(), dealHP.action(1))
        self.assertDealEqual(dealHA, dealHP)
        dealHPP = self.game.make_call(dealHP.deepcopy(), "pass")
        dealHPPP = self.game.make_call(dealHPP.deepcopy(), "pass")
        dealHPPP2 = self.game.play_card(dealHPPP.deepcopy(), "Club", "Two")
        dealHPPPA = self.game.execute_action(dealHPPP.deepcopy(), dealHPPP2.action(4))
    ...
    def test_revoke(self):
        ...
        one_notrump_deal = deal.deepcopy()
        ...
        deal = one_notrump_deal.deepcopy()
        deal = self.game.play_card(deal, "Club", "Seven")
        deal = self.game.give_card(deal, "North", "Club", "Three")
        self.assertEqual(deal.error, None)
        deal = self.game.play_card(deal, "Spade", "Ace")
